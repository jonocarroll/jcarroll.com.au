...
I'm currently building an internal tool and wanted to add some click-event-based 
...
extemely simple case, it's some `plotly` output followed by some text.
* A server function which performs the 'backend' operations,  writing to the components 
...
This is nice for interacting with the plot to, say, highlight a table containing the same 
data. With two of these plots side-by-side one can give each a unique `source` and "listen" 
to those independently.
...
to that trace and we could identify which subplot was clicked. A small example 
of such a setup might be
...
```{r addmarkers, include = FALSE, eval = FALSE}
library(plotly)
library(shiny)
...
appears to be to add another entry to the clickdata using `customdata`...
...
refinement. I really need to jump straight to that more often instead of fiddling 
