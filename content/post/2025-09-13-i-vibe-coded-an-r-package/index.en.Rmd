...
tags:
  - rstats
type: ''
subtitle: 'and it ... actually works?!?'
image: ''
---
...
resource is [Duolingo](https://www.duolingo.com/), which I think is okay, but
...
So I'm a lot of the way through the course, but it doesn't actively _teach_ me
...
I was recommended to try [WaniKani](https://www.wanikani.com/), which uses
...
Alex Chan's excellent 2020 post [Storing language vocabulary as a
graph](https://alexwlchan.net/2020/storing-language-vocabulary-as-a-graph/)
...
WaniKani API exists and that's what [Smouldering Durtles](https://github.com/jerryhcooke/smouldering_durtles) (the unofficial
Android app I occasionally use to view my progress) uses behind the scenes. All
I needed was a way to query it.

I started by trying to fetch the data myself. I [got my API
key](https://www.wanikani.com/settings/personal_access_tokens), looked up the
[documentation](https://docs.api.wanikani.com/), figured out what I needed to
fetch, and then tweaked `{wanikanir}` to use `{httr2}` (with pagination) and
...
1am that night and _only just_ managed to get the actual data in the form of a
...
files which seems to be _very_ powerful, e.g. this video which demonstrates lots 
of Claude Code's abilities

<div align="center">
<iframe width="420" height="315"
src="https://www.youtube.com/embed/d7Pb73dbcIM">
</iframe>
</div>
<div class="figcaption">How to 10x your notes with AI agents</div>

and decided to take it for a proper spin. I've previously tried using [Amp](https://ampcode.com/) to vibe code something – Geoff Huntley [demonstrated that](https://ghuntley.com/agent/) at [DataEngBytes](https://dataengbytes.com/)
...
tools (e.g. via `bash`) so I can trust that it'll run the tests, build the docs,
...
this is going to be. The entire prompt, not even using the `/init` setup, was
just
...
[wisprflow.ai/r?JONATHAN746](https://wisprflow.ai/r?JONATHAN746)
...
![Passing 133 tests](images/tests.png){width=350px}
...
`devtools::check()` succeeds with no errors or warnings and only two NOTES 
related to having a bit too large data and a `figures` directory for the vignette

![Passing check() with only minor NOTES](images/check.png){width=500px}

The package is here in case anyone's interested:

https://github.com/jonocarroll/rwanikani
...
say or point out that oh there's a `x$data$something` field. 
...
tests, but fully passing `devtools::check()`. That's so much faster than I could
...
difference from doing it manually is that I now *have* that first version, and
...
In case you missed it in the middle of the post, the link to the package is
https://github.com/jonocarroll/rwanikani - happy learning!
