...
"table" (admittedly far from the catchiest, most memorable, or most useful
name, apart from the fact that it's potentially extremely familiar to an R
user, ditto for "sapply")?
...
How far could one take this? Quite a long way, it seems. 
...
One last piece I wanted to support, though: the `where()` example above extracts 
...
but with that, and a wrapper to create an such object, the comparison operators 
can be vectorised 
...
which means that you can now run
