...
Yep, that works exactly as we hoped, but is also a built-in "filter" so we can't use it. 
...
which means that if we can _produce_ such a logical vector, say, by applying a predicate function, we can do subsetting that way 
...
Instead of using `is_odd()` we can just _negate_ the logical vector to get the same effect

```{r}
test_vec[!is_even(test_vec)]
```

We can make those into functions that take a predicate and a vector
...
It's also worth mentioning that we're passing a reference to the function `is_even` to our 
`keep` and `discard` functions - that is serving as our predicate because we need a way to 
state "is the number even?" which references the number, so we need a function. That _doesn't_ 
...
We can subset a vector with a logical vector of the same length, specifying whether or not 
to include that element, but we can _also_ subset by position (keeping in mind that R is a 
...
so we can use this with our predicate to keep elements
...
If we look at the source of `Filter()`, we see that we weren't far off of exactly 
that
...
Of course, this approach is a Bad Idea™ in general but we're not optimising anything here. This 
approach _does_ have the advantage that it isn't relying on R's vectorised capabilities, since 
each element is passed to the predicate function individually, so if we did have a non-vectorized 
...
(and vice-versa) so the previous solution is a good starting point. First, we define 
a base case; if we run out of numbers to process, return `NULL`. A convenient feature 
...
Otherwise, we can take the first value in the vector and test it with the predicate. If it 
returns `TRUE` we can append it to what we've calculated so far, and recursively 
