...
file.info(example_file, extra_cols = FALSE)[, c("size", "isdir", "mode")]
```
...
as it's created. There's sometimes a converting funtion, e.g. `as.object_size`, but I don't see 
...
The formals for this function have a default of `digits = 1L` so why is it not being found? Tracing back 
further we see that the `format.object_size` method was called from `format.data.frame` which loops through 
the columns of the `data.frame` and formats according to each class there
...
but there's no mention of `digits` here. Tracing further back, this is called from `print.data.frame` and 
...
`format.object_size` which has no way to deal with `digits = NULL`. 

Can we test that? Sure, let's create a simple `data.frame` and then make a column be `object_size`
...
consequence of this would be across all existing functions, but I can't imagine there's much 
...
`format.object_size` and call `print.data.frame` and expect it to work. What we can do, 
though, is to write a `format.object_size2`, class our new column as `object_size2`, and 
test *that*. With this new function written, we get

```{r format2}
format.object_size2 <- function(x, units = "b", standard = "auto", digits = 1L, ...)
{
  ...
  digits <- ifelse(is.null(digits), getOption("digits"), digits) # added
...
file.info2(example_file, extra_cols = FALSE)[, c("size", "isdir", "mode")]
```

It works! Are we done? What about another look at that `class(res) <- "data.frame"` business?
...
file.info3(example_file)
```

Fair enough, there's probably no `as.data.frame.object_size2` method (the 2 is for our convenience, but 
there's no `as.data.frame.object_size` either). It's simple enough to add

```{r as.data.frame.object_size, error = TRUE}
as.data.frame.object_size2 <- as.data.frame.vector
...
I wrote a patch that adds the new test for `NULL` line to the definition of 
`format.object_size` (and the corresponding bug report) and submitted that. 
I've then written another patch to the bug report this started with which 
