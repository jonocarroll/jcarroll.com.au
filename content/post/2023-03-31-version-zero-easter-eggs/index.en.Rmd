...
The next lines set up a `Date` object and extract parts
```
const datetime = new Date(1997, 7, 24);
res.push(
...
rendered as usual - in this case they're the actual javascript from the easter egg code. 
{knitr} _does_ have a way to evaluate javascript in code chunks with the `node` engine, but 
that essentially runs `node -e 'CODE'` on each chunk independently, so you can't define 
a variable in one chunk then reference it in another. That wasn't sufficient for this 
exploration. I did find an (old) [implementation](https://github.com/yihui/runr/pull/18) 
that uses [{V8}](https://github.com/jeroen/V8) in Yihui's (already experimental) 
