...
knitr::knit_engines$set(haskell = function(options) {
  ...
  writeLines(c(":set +m", ":set -XOverloadedStrings", ":set -XTemplateHaskell", "", code), con = codefile)
  out  <- system2(
    file.path(path.expand('~'), '.ghcup/bin/ghc'),
    c('-package dataframe -package text', '-e',"':script ", codefile, "'"),
    # c('-e',"':script ", codefile, "'"),
    stdout = TRUE
  )
...
`map` such that the 
[intermediate vector isn't actually used at all, e.g.](https://www.tweag.io/blog/2017-10-12-vector-package/#array-fusion).
