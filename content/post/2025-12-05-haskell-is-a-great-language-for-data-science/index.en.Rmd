...
dataHaskell's [dataframe](https://dataframe.readthedocs.io/en/latest/) package 
...
`a` rather than a _specific_ type. Technically all functions take only one argument, 
possibly returning another function (see [currying](https://en.wikipedia.org/wiki/Currying)) 
...
You can also add [rewrite rules](https://ghc.gitlab.haskell.org/ghc/doc/users_guide/exts/rewrite_rules.html)
...
Even without such a rule, Haskell (being a compiled language) is _fast_
...
**UPDATE**: [Alice](https://functional.cafe/@prophet/115698227419709447) helpfully 
explains why this runs so fast here - Haskell is lazy, so I wasn't _actually_ evaluating
anything there. Trying to calculate the length of the result _does_ take some 
time. Entirely my fault, but this comparison is unfair.
...
This ins't just about compiling; R _does_ have just-in-time compilation of 
...
because in Haskell the period is used for composition, as described [above](#syntax).
